...
const addPatient = (() => {
    ...
    btnNext1_initialConsultation.click(()=>{
        ...
        if(txtSerica.val()< 0 || txtSerica.val()> 10){
            errores = errores.concat("-Creatina Sérica fuera del rango 0 a 10\n");
        }
        if (txtDepuracion.val()<0 || txtDepuracion.val()>200 ) {
            errores = errores.concat("-Depuración creatinina fuera del rango 0 a 200\n");
        }
        if (txtECOG.val() == "" || txtECOG.val() <0 || txtECOG.val()>5  ){
            errores = errores.concat("-Asigne un valor a ECOG entre 0 y 5\n");
        }
        if (txtECOG.val()>2){
        ...
        if(txtCelulasPlasmáticasMédulaÓsea.val()>0 && txtCelulasPlasmáticasMédulaÓsea.val()<10 ){
            errores = errores.concat("-Reconciderar diagnostico Células plasmáticas menor a 10%\n");
        }
